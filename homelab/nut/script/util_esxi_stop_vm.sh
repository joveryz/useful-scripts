...
shutdown_guest_vm() {
  ...
  ssh -o connecttimeout=5 "${esxiuser}"@"${esxihost}" vim-cmd vmsvc/power.getstate $1 | grep -i "off" >/dev/null 2<&1
  l_status=$?
  ...
  if [ $l_status -eq 0 ]; then
    ...
    while [ $l_try -lt $waitretries ] && [ $l_status -ne 0 ]; do
      ...
      if [ $dryrun -ne 0 ]; then
        ...
        ssh -o connecttimeout=5 "${esxiuser}"@"${esxihost}" vim-cmd vmsvc/power.shutdown $1 >/dev/null 2<&1
    ...
    if [ $l_status -eq 0 ]; then
      ...
      if [ $dryrun -ne 0 ]; then
        ...
        ssh -o connecttimeout=5 "${esxiuser}"@"${esxihost}" vim-cmd vmsvc/power.off $1 >/dev/null 2<&1
        sleep $waitdelay
      fi
      ssh -o connecttimeout=5 "${esxiuser}"@"${esxihost}" vim-cmd vmsvc/power.getstate $1 | grep -i "off" >/dev/null 2<&1
      l_status=$?
